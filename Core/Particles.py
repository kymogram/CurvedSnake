...
class Particles:
    ...
    def __init__(self, canvas, x, y, color):
        ...
        self.particles_direction = [uniform(0, 2*pi)
                                    for i in range(nbr_particles)]
        min, max = SPEED_PARTICLES_MIN, SPEED_PARTICLES_MAX
        self.particles_speed = [uniform(min, max)
                                for i in range(nbr_particles)]
        min, max = LIFE_PARTICLES_MIN, LIFE_PARTICLES_MAX
        particles_life = [randint(min, max)
                          for i in range(nbr_particles)]
        size, life = self.particles_size, particles_life
        self.particles_life_diminution = [size[i] / life[i]
                                          for i in range(nbr_particles)]
        ...
        for index, particle in enumerate(self.particles):
            ...
            if self.particles_size[index] < 1:
                ...
                for i in range(len(pos)):
                    size = self.particles_size[index]
                    if i == 0 or i == 2:  # X coordinate
                        pos[i] += x_offset
                    # y0
                    elif i == 1:
                        pos[i] += y_offset-(size-size_save)/2
                    # y1
                    else:
                        pos[i] += y_offset+(size-size_save)/2
                # apply offset
